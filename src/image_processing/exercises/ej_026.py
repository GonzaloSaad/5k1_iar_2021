...
axes[1].imshow(accumulator, extent=bounds, cmap=cm.gray, aspect=1 / 1.5)
...
for theta_index, r_index, acc in sorted_points[:4]:
    theta = tested_angles[theta_index]
    r = (r_index - medium_r_index) * radius_precision
    (x0, y0) = r * np.array([np.cos(theta), np.sin(theta)])
    axes[2].axline((x0, y0), slope=np.tan(theta + np.pi / 2), color='red', linewidth='1.1', linestyle='dashed')
